...
if [ -e "$HOME/../usr/bin/ruby" ]; then
	echo -e " ${GREEN}ruby detectado prosiguiendo..."
	sleep 1
	if [ -e "/data/data/com.termux/files/usr/bin/lolcat" ]; then
		echo -e " ${GREEN}lolcat detectado prosiguiendo..."
		sleep 1
	else
	...
	echo -e " ${RED}No instaló ruby, instalando ..."
	sleep 1 
...
if [ -e "$HOME/../usr/bin/nodejs" ]; then
	echo -e " ${GREEN}nodejs detectado prosiguiendo..."
	sleep 1
	else
...
if [ -e "$HOME/../usr/bin/libwebp" ]; then
	echo -e " ${GREEN}libwebp detectado prosiguiendo..."
	sleep 1
	else
		echo -e " ${RED} libwebp no detectado, instalando..."
echo -e " ${NOCOLOR}"
...
if [ -e "$HOME/../usr/bin/mc" ]; then
	echo -e " ${GREEN}mc detectado prosiguiendo..."
	sleep 1
	else
		echo -e " ${RED} mc no detectado, instalando..."
echo -e " ${NOCOLOR}"
echo -e " ${PURPLE} Usando el comando: apt-get install mc"
echo -e " ${GREEN}"
sleep 5
...
if [ -e "$HOME/../usr/bin/ffmpeg" ]; then
	echo -e " ${GREEN}ffmpeg detectado prosiguiendo..."
	sleep 1
	else
...
if [ -e "$HOME/../usr/bin/wget" ]; then
	echo -e " ${GREEN}wget detectado prosiguiendo..."
	sleep 1
	else
...
echo -e " ${NOCOLOR}"
echo -e " ${PURPLE} Limpiando cache npm..."
echo -e " ${GREEN}"
sleep 5
...
echo -e " ${YELLOW} Usa el comando: ${GREEN}npm start ${YELLOW}para iniciar el bot."
